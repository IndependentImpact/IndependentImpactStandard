# shape2form Workflow Definitions

This directory contains YAML files that define the activity workflows supported
by the NIAS shape2form integration. Each file describes one workflow: which
...
*-ui-shapes.ttl  (activity-specific adapter bundle)
        │
        │  shape2form build script reads the bundle
...
3. **UI adapter bundles** (e.g. `pdd-design-ui-shapes.ttl`) flatten inherited
   shapes and add `ui:` hints so `shape2form` can produce usable Flutter
   forms.
4. **Build scripts** (e.g. `build-pdd-design.sh`) call `shape2form build`
   with the adapter bundle and write generated Dart/schema artifacts to
   `/tmp/nias-shape2form/…` outside the repository.
5. **Export tools** (e.g.
   `shape2form/validation_report/tool/export_validation_report_markdown.py`)
...
standard and the shape2form toolchain. They are read by the automated tests
...
```
dataRequirements/shape2form/workflows/<activity-name>.yaml
```
...
`dataRequirements/shape2form/` that flattens and annotates the shapes for
`shape2form`. Point `ui_shape_bundle` at that file.
...
- call `shape2form build` with the new `*-ui-shapes.ttl` bundle,
- write output to a dedicated path under `/tmp/nias-shape2form/<activity>/`.
...
Add a section to `dataRequirements/shape2form/README.md` describing the new
...
canonical_bundle:
  ui_shape_bundle: dataRequirements/shape2form/my-new-report-ui-shapes.ttl
...
- `dataRequirements/shape2form/README.md` — overall shape2form integration guide
- `dataRequirements/shape2form/*-ui-shapes.ttl` — activity-specific adapter bundles
- `dataRequirements/shape2form/build-*.sh` — build scripts that call `shape2form`
