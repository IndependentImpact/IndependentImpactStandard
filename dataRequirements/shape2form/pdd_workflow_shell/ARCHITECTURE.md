...
**Document scope:** This paper analyses the `dataRequirements/shape2form/pdd_workflow_shell`
Flutter package. It documents how the shell composes generated shape2form Dart
...
2. **UI adapter bundle (derived, non-canonical):**
   `dataRequirements/shape2form/pdd-workflow-ui-shapes.ttl` projects those
...
| 3. shape2form validation | `build-pdd-workflow.sh` → `shape2form lint` | UI adapter bundle | Lint verdict for projection correctness |
| 4. Form schema emit | `build-pdd-workflow.sh` → `shape2form emit-jsonld` | UI adapter bundle | `/tmp/nias-shape2form/pdd-workflow/schema/forms.jsonld` |
| 5. Dart generation | `build-pdd-workflow.sh` → `shape2form build` | UI adapter bundle | `/tmp/nias-shape2form/pdd-workflow/flutter/*.dart` |
...
```bash
"$SHAPE2FORM_BIN" lint   -allow-path-prefixes "$ALLOWED_PREFIXES" "$SHAPES_FILE"
"$SHAPE2FORM_BIN" emit-jsonld -format jsonld -o "$SCHEMA_DIR/forms.jsonld" "$SHAPES_FILE"
"$SHAPE2FORM_BIN" build  -outdir "$FLUTTER_DIR" "$SHAPES_FILE"
```

`$SHAPES_FILE` is `dataRequirements/shape2form/pdd-workflow-ui-shapes.ttl`.
Outputs land in `/tmp/nias-shape2form/pdd-workflow/{schema,flutter}` (or the
path set by `$OUT_ROOT`).

### 3.4 Prepare script (`tool/prepare_pdd_workflow_shell.sh`)

```bash
"$ROOT_DIR/dataRequirements/shape2form/build-pdd-workflow.sh"  # run shape2form from adapter SHACL
...
| Change which forms are generated and where they are written | `build-pdd-workflow.sh` shape2form args/env | Different generated schema/Dart artifacts; shell prepare script consumes new output |
...
`dataRequirements/shape2form/pdd-workflow-gate.md`. The shell implements
...
Run `dataRequirements/shape2form/build-monitoring-report.sh`. This produces
`MonitoringReportUiShapeFormWidget` (and supporting subform widgets) in
`/tmp/nias-shape2form/monitoring-report/flutter/`.
...
| `../build-pdd-workflow.sh` | Runs shape2form lint / emit-jsonld / build |
