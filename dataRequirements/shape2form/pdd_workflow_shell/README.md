...
This Flutter package hosts the generated PDD shape2form forms in a workflow
shell.
...
| `shape2form` binary | any recent build | set `SHAPE2FORM_BIN` in `.env` |
...
|----------|---------|---------|
| `SHAPE2FORM_BIN` | *(must be set)* | Absolute path to the `shape2form` binary |
...
`/tmp/nias-shape2form/pdd-workflow/flutter`.
